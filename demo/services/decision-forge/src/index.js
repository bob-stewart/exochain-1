  ...
  return async (req, res) => {
    ...
    try {
      ...
      if (url.pathname === '/api/decision/create' && req.method === 'POST') {
        ...
        const decision = wasmDep.wasm_create_decision(title, JSON.stringify(decision_class || 'Operational'), hash);
      ...
      if (url.pathname === '/api/decision/vote' && req.method === 'POST') {
        ...
        const updated = wasmDep.wasm_add_vote(JSON.stringify(decision_json), JSON.stringify(vote));
      ...
      if (url.pathname === '/api/decision/evidence' && req.method === 'POST') {
        ...
        const updated = wasmDep.wasm_add_evidence(JSON.stringify(decision_json), JSON.stringify(evidence));
        return json(res, 200, updated);
      }
      ...
      if (url.pathname === '/api/decision/transition' && req.method === 'POST') {
        ...
        const updated = wasmDep.wasm_transition_decision(JSON.stringify(decision_json), JSON.stringify(to_state), actor_did);
        return json(res, 200, updated);
      }
      ...
      if (url.pathname === '/api/decision/hash' && req.method === 'POST') {
        ...
        const hash = wasmDep.wasm_decision_content_hash(JSON.stringify(decision_json));
      ...
      if (url.pathname === '/api/decision/terminal' && req.method === 'POST') {
        const { decision_json } = await parseBody(req);
        const terminal = wasmDep.wasm_decision_is_terminal(JSON.stringify(decision_json));
      ...
      if (url.pathname === '/api/decision/challenge' && req.method === 'POST') {
        ...
        const challenge = wasmDep.wasm_file_challenge(challenger_did, decision_id, JSON.stringify(ground), evidence_hash);
      ...
      if (url.pathname === '/api/decision/accountability' && req.method === 'POST') {
        ...
        const action = wasmDep.wasm_propose_accountability(target_did, proposer_did, JSON.stringify(action_type), reason, evidence_hash);
