  ...
  return async (req, res) => {
    ...
    try {
      ...
      if (url.pathname === '/api/users' && req.method === 'GET') {
        const { rows } = await poolDep.query('SELECT did, display_name, email, roles, status, pace_status FROM users');
        return json(res, 200, rows);
      }

      // ── Identity Scores ──
      if (url.pathname === '/api/scores' && req.method === 'GET') {
        const { rows } = await poolDep.query('SELECT i.*, u.display_name FROM identity_scores i JOIN users u ON u.did = i.did');
        return json(res, 200, rows);
      }

      // ── Enrollment Log ──
      if (url.pathname === '/api/enrollment' && req.method === 'GET') {
        const { rows } = await poolDep.query('SELECT * FROM enrollment_log ORDER BY timestamp DESC');
        return json(res, 200, rows);
      }
      ...
      if (url.pathname === '/api/shamir/split' && req.method === 'POST') {
        ...
        const result = wasmDep.wasm_shamir_split(secretBytes, threshold || 2, shares || 3);
      ...
      if (url.pathname === '/api/shamir/reconstruct' && req.method === 'POST') {
        ...
        const result = wasmDep.wasm_shamir_reconstruct(JSON.stringify(shares), threshold, total);
      ...
      if (url.pathname === '/api/pace/resolve' && req.method === 'POST') {
        ...
        const result = wasmDep.wasm_pace_resolve(JSON.stringify(config), JSON.stringify(state));
        return json(res, 200, result);
      }
      ...
      if (url.pathname === '/api/pace/escalate' && req.method === 'POST') {
        ...
        const result = wasmDep.wasm_pace_escalate(JSON.stringify(state));
      ...
      if (url.pathname === '/api/risk/assess' && req.method === 'POST') {
        ...
        const result = wasmDep.wasm_assess_risk(
