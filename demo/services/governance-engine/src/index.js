  ...
  return async (req, res) => {
    ...
    try {
      ...
      if (url.pathname === '/api/quorum/compute' && req.method === 'POST') {
        ...
        const result = wasmDep.wasm_compute_quorum(JSON.stringify(approvals), JSON.stringify(policy));
      ...
      if (url.pathname === '/api/clearance/check' && req.method === 'POST') {
        ...
        const result = wasmDep.wasm_check_clearance(actor_did, action, JSON.stringify(policy));
        return json(res, 200, result);
      }
      ...
      if (url.pathname === '/api/conflicts/check' && req.method === 'POST') {
        ...
        const result = wasmDep.wasm_check_conflicts(actor_did, JSON.stringify(action), JSON.stringify(declarations || []));
        return json(res, 200, result);
      }
      ...
      if (url.pathname === '/api/challenge' && req.method === 'POST') {
        ...
        const result = wasmDep.wasm_file_governance_challenge(
          challenger_did,
          target_hash,
      ...
      if (url.pathname === '/api/authority/build' && req.method === 'POST') {
        ...
        const chain = wasmDep.wasm_build_authority_chain(JSON.stringify(links));
      ...
      if (url.pathname === '/api/evaluate' && req.method === 'POST') {
        ...
        // Load decision
        const { rows: [decision] } = await poolDep.query('SELECT * FROM decisions WHERE id_hash = $1', [decision_id]);
        if (!decision) return json(res, 404, { error: 'Decision not found' });

        // Load actor
        const { rows: [actor] } = await poolDep.query('SELECT * FROM users WHERE did = $1', [actor_did]);
        if (!actor) return json(res, 404, { error: 'Actor not found' });

        // Check clearance
        const clearancePolicy = { actions: { [decision.status]: { required_level: 'Governor' } } };
        const clearance = wasmDep.wasm_check_clearance(actor_did, decision.status, JSON.stringify(clearancePolicy));

        // Check conflicts
        const action = { action_id: decision_id, actor_did, affected_dids: [], description: `Evaluate ${decision_id}` };
        const conflicts = wasmDep.wasm_check_conflicts(actor_did, JSON.stringify(action), '[]');

        // Load delegations for authority chain
        const { rows: delegations } = await poolDep.query('SELECT * FROM delegations WHERE delegatee = $1 AND expires_at > $2', [actor_did, Date.now()]);
